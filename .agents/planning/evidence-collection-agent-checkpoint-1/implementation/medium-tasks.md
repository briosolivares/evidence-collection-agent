...
- **Q (M1 resolution, via Tina): what is ground truth for "top 30 contributors" — the website's contributors graph or the REST API?** A: **the website is a valid, correct source** — the agent is testing browser use and must not be steered to APIs (a proposed API-first prompt norm and exact-match "drift collar" grader were dropped before implementation). Bot/agent accounts in the list (`claude`, `cursoragent`, `[bot]` accounts) are **accepted as correct**; a human-filtered reading would also be acceptable (the LinkedIn column implies humans), so the grader must pass both. Task text unchanged. Implementation: `CONTRIBUTOR_WINDOW_SIZE` 40 → 100 (tolerance ≥25 of 30 unchanged) — the wider API window absorbs the site-vs-API rank spread (site ~23–30 ↔ API 41–70); the tolerance absorbs co-author-credited accounts the REST API cannot see at any window size. Verified by regrading the 2026-08-11 evening runs: 3/3 trials 7/7, task PASS (`evals/experiments/2026-08-11_10-18-13pm_eval-openclaw-contributors_0d95c0.json`).
- **Deferred UX note (user, 2026-08-11):** a nicer product behavior for ambiguous asks like "top contributors" might be the agent asking a clarifying follow-up question before running. Explicitly not a focus now.
...
- **Mechanism ladder since the baseline** (each has a report in `docs/reports/`): ceiling raise (Step 0) → moving cache breakpoint + 200k per-request context guard, commit `8d0ea98` (Step 2: first 8/8 trials; 60-turn cap the last constraint) → retry on transients + `maxTurns` off, commit `fced2f7`.
- **Latest run (2026-08-11 evening, retry + maxTurns off): still 0/2 strict, but merged PRs went 8/8 at 76 turns and contributors completed its 30-row CSV in all three trials (85–94 turns).** Contributors now fails only a deterministic page-vs-API ranking gap — the tolerance above (≥25 of 30 in the top-40 window) undershoots the real disagreement (22 observed, same 8 bot/edge handles every trial) — an **eval-design decision now pending**, not an agent defect. Report + open queue (M1 ranking gap, M2 oracle freshness churn, M3 context exhaustion): `docs/reports/2026-08-11-retry-maxturns-off.md`.
