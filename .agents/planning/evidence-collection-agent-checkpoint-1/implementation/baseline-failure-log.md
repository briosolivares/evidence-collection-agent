...
- **Result:** Applied by user decision: the production default is now 24 turns; the configurable 250k cumulative token ceiling is unchanged. A regression test verifies that a run can complete on turn 24 when `maxTurns` is omitted. **Re-baseline: verified.** No trial ended `budget_exceeded`. EDGAR trials completed in 10–11 turns including read-back verification and exhibit downloads — the recovery loop that previously hit the 12-turn wall now fits with headroom.
