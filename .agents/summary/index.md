...
| [interfaces.md](interfaces.md) | The internal seams (`BrowserController`, `BrowserSessionProvider`, `CallModel`, `ToolDef`, `runTask`, `RunTracing`), eval contracts (`Grader`, `EvalTask`, task.json schema, CLI flags), external services, env vars | Adding a tool/task/browser provider; wiring questions; API/env details |
