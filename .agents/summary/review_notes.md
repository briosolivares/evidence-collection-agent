...
Review snapshot for the active v3-only summaries as of 2026-08-15. Historical planning and report documents are intentionally not rewritten when architecture changes; use their dates and links as context, not as current module maps.

## Consistency findings

1. **Production has one v3 path.** `src/cli/runTask.ts` composes the initializer, sequential worker, deterministic finish checks, fresh verifier, static eight-tool registry, and v3 coordinator. The old checkpoint-1 execution modules and typed output/evidence stores are no longer production dependencies.
2. **The cached worker prefix is static.** `V3_SYSTEM_PROMPT` and `V3_API_TOOL_DEFS` hold no task/run/provider state. Per-run facts enter conversation guidance and the durable configuration.
...
4. **Browser ownership is explicit.** The local TUI attaches to user-owned Chrome, while the REPL and both eval lanes use managed sessions. Browserbase is selected only by the provider variable; possessing a key alone does not start a remote session.
...
- **Terminal resume is intentionally quiet.** It must not construct real model clients, perform browser work, or start a new external root trace. It only revalidates checkpoint/run integrity and repairs projections under lock.
