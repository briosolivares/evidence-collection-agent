...
2. **Per-trial grading (M2)** — grade each run right after it finishes instead of after the whole suite; also shrinks the blast radius of any grading-time failure (compounds with Part A's retry).
