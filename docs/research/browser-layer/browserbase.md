...
Build a thin `BrowserbaseBrowserSessionProvider` that returns the existing `BrowserController` contract. Use a non-production tenant and synthetic records. For each run, emit `{task_id, session_id, region, URL, action log, artifact SHA-256, source timestamp}` and copy artifacts to local/S3-compatible controlled storage.
