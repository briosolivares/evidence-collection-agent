...
describe('PlaywrightBrowserController task-page ownership', () => {
  ...
  it('closes task, popup, and raw-created pages while preserving user pages', async () => {
    ...
    const taskPage = context.pages().find((page) => !preexistingPages.includes(page));
    ...
    expect(context.pages()).toEqual([...preexistingPages, concurrentUserPage]);
  }, 15_000);

  it('reclaims only same-run pages after an attached-client reconnect', async () => {
    ...
    const taskPage = context.pages().find((page) => !userPages.includes(page));
    expect(taskPage).toBeDefined();
    ...
    await Promise.all([
      taskPage!.goto('data:text/html,<title>cross-origin-main</title><h1>main</h1>'),
      popupPage.goto('data:text/html,<title>cross-origin-popup</title><h1>popup</h1>'),
    ]);
    ...
    const concurrentUserPage = await context.newPage();
    await concurrentUserPage.goto('data:text/html,<title>concurrent-user</title><h1>user</h1>');
  ...
  it('closes a real navigating task page on preparation cancellation while preserving user tabs', async () => {
    const durableRunId = 'run-2026-08-15-reconnect-test';
    const userPages = [...context.pages()];
