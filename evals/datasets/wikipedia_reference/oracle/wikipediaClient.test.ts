...
import { fetchWikipediaReference, htmlToText, parseReferenceSourceHtml } from './wikipediaClient.js';
...
describe('Wikipedia reference oracle', () => {
  ...
  it('throws when the displayed reference or Sources link is absent', () => {
    ...
    expect(() => parseReferenceSourceHtml(HTML.replace('#CITEREFBeevor2012', '#not-a-source'))).toThrow(/does not link/);
  });
  ...
  it('fetches through the official parse API and validates its envelope', async () => {
    ...
    const fetchFn = async (input: string | URL | Request): Promise<Response> => {
      ...
      return new Response(JSON.stringify({ parse: { title: 'World War II', text: HTML } }), { status: 200 });
    };
    const result = await fetchWikipediaReference({ fetchFn, sleep: async () => undefined, random: () => 0.5 });
    expect(result.sourceId).toBe('CITEREFBeevor2012');
    expect(seen[0]).toContain('action=parse');

    await expect(fetchWikipediaReference({ fetchFn: async () => new Response('{}'), sleep: async () => undefined }))
