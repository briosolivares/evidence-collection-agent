...
describe('wikipedia_reference grader', () => {
  ...
  it('rejects the short reference text in place of the linked Sources entry', async () => {
    writeArtifact(runDir, 'artifacts/answer.md', Buffer.from('Beevor 2012, pp. 555–560.'), {
      roles: ['requested_output'],
    });
    const results = await grade(runDir, ORACLE);
    expect(
      byName(results, 'answer contains the complete source text reached from reference 275').passed,
    ).toBe(false);
  });

  it('rejects explicit truncation even if the source text is present', async () => {
    writeArtifact(runDir, 'artifacts/answer.md', Buffer.from(`${ORACLE.sourceText} ...`), {
      roles: ['requested_output'],
    });
    expect(
      byName(
