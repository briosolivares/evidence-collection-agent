...
export const grade: Grader = (runDirPath, oracleData) => {
  ...
  if (!csvExists) {
    return [
      ...assertions,
      ...failedCsvContent(`${CSV_FILENAME} is unavailable`),
...
function sheetAssertion(
  ...
  const urls =
    text.match(/https?:\/\/docs\.google\.com\/spreadsheets\/d\/[A-Za-z0-9_-]+[^\s)>\]}]*/gi) ?? [];
  return {
    name: SHEET_ASSERTION_NAME,
    passed: urls.length > 0,
    detail: urls.length
      ? `found ${urls[0]}`
      : 'no docs.google.com/spreadsheets/d/<id> URL found (private sheet contents remain human-reviewed)',
  };
}
...
function resolveRows(
  header: string[],
  rawRows: string[][],
  oracle: MitSororitiesOracle,
): MemberRow[] {
  const index = (name: string): number =>
    header.findIndex((cell) => cell.trim().toLowerCase() === name);
  const cell = (row: string[], i: number): string => (i < 0 ? '' : (row[i] ?? '').trim());
  const indices = Object.fromEntries(REQUIRED_COLUMNS.map((name) => [name, index(name)])) as Record<
...
function cohortAssertion(rows: MemberRow[], oracle: MitSororitiesOracle): AssertionResult {
  ...
  if (rows.length < oracle.minRows || rows.length > oracle.maxRows)
    problems.push(`found ${rows.length} rows, expected ${oracle.minRows}–${oracle.maxRows}`);
  const invalidClass = rows
    .filter((row) => row.classYear === undefined)
    .map((row) => row.rowNumber);
  const invalidAffiliation = rows
    .filter((row) => row.affiliation === undefined)
    .map((row) => row.rowNumber);
  if (invalidClass.length) problems.push(`invalid class in row(s) ${invalidClass.join(', ')}`);
  if (invalidAffiliation.length)
    problems.push(`invalid affiliation in row(s) ${invalidAffiliation.join(', ')}`);
  const isOptional = (affiliation: string, classYear: number): boolean =>
    oracle.optionalCohorts.some(
      (cohort) => cohort.affiliation === affiliation && cohort.classYear === classYear,
    );
  const absent = oracle.affiliations.flatMap((affiliation) =>
    oracle.classes
      .filter(
        (classYear) =>
          !rows.some((row) => row.affiliation === affiliation && row.classYear === classYear),
      )
      .map((classYear) => ({
        label: `${affiliation} ${classYear}`,
        optional: isOptional(affiliation, classYear),
      })),
  );
  ...
  const waivedNote = waived.length
    ? `; absent but optional (unsourced on the live web): ${waived.join(', ')}`
    : '';
  const requiredCount =
    oracle.affiliations.length * oracle.classes.length - oracle.optionalCohorts.length;
  return {
    ...
    detail:
      (problems.length
        ? problems.join('; ')
        : `${rows.length} rows cover all ${requiredCount} required cohorts`) + waivedNote,
  };
}

function identityAssertion(rows: MemberRow[]): AssertionResult {
  const invalid = rows
    .filter((row) => !/^[\p{L}][\p{L}'’.\-]+(?:\s+[\p{L}][\p{L}'’.\-]+)+$/u.test(row.name))
    .map((row) => row.rowNumber);
  const keys = rows
    .filter((row) => row.affiliation && row.name)
    .map((row) => `${row.affiliation}|${row.name.toLowerCase().replace(/\s+/g, ' ')}`);
  ...
  return {
    ...
    detail: problems.length
      ? problems.join('; ')
      : `${rows.length} plausible, affiliation-unique names`,
  };
}

function detailAssertion(rows: MemberRow[], oracle: MitSororitiesOracle): AssertionResult {
  const majorCoverage = rows.length
    ? rows.filter((row) => !isEmptyish(row.major)).length / rows.length
    : 0;
  const enrichmentCoverage = rows.length
    ? rows.filter((row) => !isEmptyish(row.interests) || !isEmptyish(row.other)).length /
      rows.length
    : 0;
  const passed =
    majorCoverage >= oracle.minMajorCoverage && enrichmentCoverage >= oracle.minEnrichmentCoverage;
  return {
...
function normalizeClass(value: string, accepted: readonly number[]): number | undefined {
  ...
  const year = accepted.find((candidate) =>
    new RegExp(`(?:class\\s+of\\s+)?${candidate}`).test(lower),
  );
  if (year) return year;
...
function isEmptyish(value: string): boolean {
  return ['', '-', 'n/a', 'na', 'none', 'unknown', 'not found'].includes(
...
function failedCsvContent(detail: string): AssertionResult[] {
  return [
    ...
    COHORT_ASSERTION_NAME,
    IDENTITY_ASSERTION_NAME,
    DETAIL_ASSERTION_NAME,
...
function asOracle(data: unknown): MitSororitiesOracle {
  ...
  if (
    ...
    value.affiliations.length !== 6 ||
    !value.affiliations.every((item) => typeof item === 'string') ||
    !Array.isArray(value.classes) ||
    value.classes.length !== 2 ||
    !Array.isArray(value.optionalCohorts) ||
    !value.optionalCohorts.every(
      (item) => typeof item?.affiliation === 'string' && typeof item?.classYear === 'number',
    ) ||
    typeof value.minRows !== 'number' ||
    typeof value.maxRows !== 'number' ||
    typeof value.minMajorCoverage !== 'number' ||
    typeof value.minEnrichmentCoverage !== 'number'
  ) {
