...
describe('company freshness oracle', () => {
  ...
  it('fetches all three official sources and Notion article dates without live HTTP', async () => {
    const fetchFn = async (input: string | URL | Request): Promise<Response> => {
      ...
      if (url.endsWith('/en-us/blog'))
        return new Response('<a href="/blog/new-notion">new</a><a href="/blog/old-notion">old</a>');
      if (url.endsWith('/blog/new-notion'))
        return new Response(
          '<meta property="article:published_time" content="2026-08-10T10:00:00Z"><meta property="og:title" content="New Notion">',
        );
      if (url.endsWith('/blog/old-notion'))
        return new Response(
          '<script type="application/ld+json">{"datePublished":"2026-08-01T10:00:00Z"}</script><title>Old Notion</title>',
        );
      if (url.endsWith('/feed/feed.json'))
        return new Response(
          JSON.stringify({
            items: [
              {
      ...
      if (url === 'https://www.eightsleep.com/blog/')
        return new Response(
          '<h3><a href="/blog/new-eight">New Eight</a></h3><p>summary</p><small>Aug 8, 2026</small>',
        );
      return new Response('not found', { status: 404 });
    };
    const result = await fetchCompanyFreshnessOracle({
      fetchFn,
