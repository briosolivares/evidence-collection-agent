...
async function fetchNotionCandidates(deps: CompanyContentDeps): Promise<ContentCandidate[]> {
  ...
  const urls = extractUniqueMatches(html, /href="(\/blog\/(?!topic\/)[a-z0-9-]+)"/gi)
    .slice(0, 10)
    .map((path) => new URL(path, 'https://www.notion.com').href);
  const candidates = (await Promise.all(urls.map((url) => fetchDatedArticle(url, deps)))).filter(
    (value): value is ContentCandidate => value !== undefined,
  );
...
async function fetchFigmaCandidates(deps: CompanyContentDeps): Promise<ContentCandidate[]> {
  ...
  const candidates = value.items.flatMap((item): ContentCandidate[] => {
    ...
    const publishedAt =
      typeof row.date_published === 'string' ? row.date_published : row.date_modified;
...
export function parseEightSleepIndex(html: string): ContentCandidate[] {
  const pattern =
    /<h3\b[^>]*>[\s\S]*?<a\b[^>]*href="(\/blog\/[a-z0-9-]+)"[^>]*>([\s\S]*?)<\/a>[\s\S]*?<\/h3>/gi;
...
async function fetchDatedArticle(
  url: string,
  deps: CompanyContentDeps,
): Promise<ContentCandidate | undefined> {
  const response = await fetchWithRetry(url, undefined, deps);
  if (!response.ok) return undefined;
  const html = await response.text();
  const publishedAt =
    /property="article:published_time"\s+content="([^"]+)"/i.exec(html)?.[1] ??
...
async function fetchText(url: string, deps: CompanyContentDeps): Promise<string> {
  const response = await fetchWithRetry(url, undefined, deps);
  if (!response.ok)
    throw new Error(`official content index request failed for ${url}: HTTP ${response.status}`);
  return response.text();
}

export function newest(candidates: ContentCandidate[], label: string): ContentCandidate[] {
  const sorted = dedupeByUrl(candidates)
    .filter((candidate) => validDate(candidate.publishedAt))
    .sort((a, b) => Date.parse(b.publishedAt) - Date.parse(a.publishedAt))
    .slice(0, CHURN_WINDOW);
...
function dedupeByUrl(candidates: ContentCandidate[]): ContentCandidate[] {
  return [
    ...new Map(candidates.map((candidate) => [normalizeUrl(candidate.url), candidate])).values(),
  ];
}
