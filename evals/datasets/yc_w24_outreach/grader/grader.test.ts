...
const ORACLE: YcW24AiOracle = {
  companies: COMPANY_NAMES.map((name, index) => ({
    ...
    founders: [
      `Alice ${['Archer', 'Baker', 'Cole', 'Diaz', 'Evans'][index]}`,
      `Bob ${['Able', 'Bell', 'Clark', 'Dunn', 'Earl'][index]}`,
    ],
  })),
};
let runDir: string;
...
function passingCsv(): string {
  const rows = ORACLE.companies.flatMap((company, index) =>
...
describe('yc_w24_outreach grader', () => {
  it('passes all founders from exactly five live-oracle companies', async () => {
    writeArtifact(runDir, 'artifacts/outreach.csv', Buffer.from(passingCsv()), {
  ...
  it('rejects an unknown founder and a missing cofounder', async () => {
    ...
    expect(
      byName(results, 'every founder belongs to an oracle-listed YC W24 AI company').passed,
    ).toBe(false);
    expect(
      byName(
        results,
        'rows represent exactly five companies and include every oracle-listed founder for each',
      ).passed,
    ).toBe(false);
  });
  ...
  it('accepts opaque personal handles but rejects duplicate profile URLs', async () => {
    ...
    const opaque = passingCsv().replace(
      'https://www.linkedin.com/in/alice-archer',
      'https://www.linkedin.com/in/xq7',
    );
    writeArtifact(runDir, 'artifacts/outreach.csv', Buffer.from(opaque), {
      roles: ['requested_output'],
    });
    expect(byName(await grade(runDir, ORACLE), linkedinAssertion).passed).toBe(true);

    const duplicate = passingCsv().replace(
      'https://www.linkedin.com/in/alice-archer',
      'https://www.linkedin.com/in/bob-able',
    );
    writeArtifact(runDir, 'artifacts/outreach.csv', Buffer.from(duplicate), {
      roles: ['requested_output'],
    });
  ...
  it('rejects a company LinkedIn URL and a generic email without the call ask', async () => {
    const bad = passingCsv()
      .replace(
        'https://www.linkedin.com/in/alice-archer',
        'https://www.linkedin.com/company/atlas-labs',
      )
      .replace(
        /"Hi Alice,[^"]+"/,
        '"Hello there. This is a generic note with no specific request and not enough useful personalization for its intended recipient."',
      );
    writeArtifact(runDir, 'artifacts/outreach.csv', Buffer.from(bad), {
      roles: ['requested_output'],
    });
    const results = await grade(runDir, ORACLE);
    expect(byName(results, 'every LinkedIn URL is a distinct personal profile URL').passed).toBe(
      false,
    );
    expect(
      byName(
  ...
  it('enforces exact columns, manifests, hashes, and oracle validation', async () => {
    writeArtifact(
      runDir,
      'artifacts/outreach.csv',
      Buffer.from(passingCsv().replace('cold_outreach_email', 'cold_outreach_email,company')),
      { roles: ['requested_output'] },
    );
    expect(
      byName(
        await grade(runDir, ORACLE),
        'CSV has exactly the columns founder_name, linkedin_url, cold_outreach_email (no more, no fewer)',
      ).passed,
    ).toBe(false);
    writeArtifact(runDir, 'artifacts/outreach.csv', Buffer.from(passingCsv()), {
      roles: ['requested_output'],
