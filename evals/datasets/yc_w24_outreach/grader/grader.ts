...
function matchRows(rows: OutreachRow[], oracle: YcW24AiOracle): MatchedRow[] {
  ...
  for (const company of oracle.companies) {
    for (const founder of company.founders)
      byFounder.set(normalize(founder), { company, canonicalFounder: founder });
  }
...
function rowAssertion(rows: OutreachRow[]): AssertionResult {
  ...
  const empty = rows
    .filter((row) => !row.founderName || !row.linkedinUrl || !row.email)
...
function founderAssertion(rows: OutreachRow[], matched: MatchedRow[]): AssertionResult {
  ...
  const unknown = rows
    .filter((row) => !matchedRows.has(row.rowNumber))
    .map((row) => `row ${row.rowNumber}: ${row.founderName || '(empty)'}`);
  return {
    name: FOUNDER_ASSERTION_NAME,
    passed: rows.length > 0 && unknown.length === 0,
    detail: unknown.length
      ? `not found in live YC W24 AI founder oracle: ${unknown.join('; ')}`
      : `${matched.length} founder(s) matched`,
  };
}
...
function companyAssertion(matched: MatchedRow[]): AssertionResult {
  ...
  for (const company of companies.values()) {
    const present = new Set(
      matched
        .filter((item) => item.company.slug === company.slug)
        .map((item) => normalize(item.canonicalFounder)),
    );
    for (const founder of company.founders)
      if (!present.has(normalize(founder))) missing.push(`${company.name}: ${founder}`);
  }
  ...
  return {
    ...
    passed,
    detail:
      `${companies.size} distinct compan${companies.size === 1 ? 'y' : 'ies'}` +
      (missing.length
        ? `; missing founder row(s): ${missing.join(', ')}`
        : '; all public founders represented'),
  };
}
...
function linkedinAssertion(rows: OutreachRow[]): AssertionResult {
  ...
  return {
    ...
    detail: bad.length
      ? bad.join('; ')
      : `${urls.length} distinct personal linkedin.com/in URLs (profile ownership is human-reviewed)`,
  };
}
...
function emailAssertion(rows: OutreachRow[], matched: MatchedRow[]): AssertionResult {
  ...
  if (new Set(normalizedEmails).size !== normalizedEmails.length)
    bad.push('duplicate outreach email(s)');
  return {
...
function failedContent(detail: string): AssertionResult[] {
  return [
    ...
    ROW_ASSERTION_NAME,
    FOUNDER_ASSERTION_NAME,
    COMPANY_ASSERTION_NAME,
    LINKEDIN_ASSERTION_NAME,
    EMAIL_ASSERTION_NAME,
  ...
  const valid =
    ...
    companies.every(
      (company) =>
        ...
        typeof (company as { slug?: unknown }).slug === 'string' &&
        Array.isArray((company as { founders?: unknown }).founders) &&
        (company as { founders: unknown[] }).founders.length > 0 &&
        (company as { founders: unknown[] }).founders.every(
          (founder) => typeof founder === 'string',
        ),
    );
