...
export async function fetchYcW24AiCompanies(deps: YcClientDeps = {}): Promise<YcW24AiOracle> {
  ...
  const [companyPayload, founderPayload] = await Promise.all([
    ...
    algoliaSearch(founderCredentials, FOUNDER_INDEX, {
      query: '', hitsPerPage: 1000, facetFilters: [['batches:W24']],
...
async function fetchDirectoryCredentials(url: string, deps: YcClientDeps): Promise<AlgoliaCredentials> {
  ...
  if (!response.ok) throw new Error(`YC directory request failed for ${url}: HTTP ${response.status}`);
...
export function parseAlgoliaCredentials(html: string): AlgoliaCredentials {
  ...
  if (!credentials || typeof credentials.app !== 'string' || !/^[A-Z0-9]+$/i.test(credentials.app) ||
      typeof credentials.key !== 'string' || credentials.key.length < 20) {
...
export function combineDirectoryResults(companyPayload: unknown, founderPayload: unknown): YcW24AiOracle {
  ...
  const companies = companyHits.flatMap((hit): YcAiCompany[] => {
  ...
  if (companies.length < 5) throw new Error(`YC oracle found only ${companies.length} W24 AI companies with public founders`);
  return { companies };
}
