...
import {
  combineDirectoryResults,
  fetchYcW24AiCompanies,
  isAiFocused,
  parseAlgoliaCredentials,
} from './ycClient.js';

const COMPANY_HITS = Array.from({ length: 5 }, (_, index) => ({
  name: `Company ${index + 1}`,
  slug: `company-${index + 1}`,
  one_liner: `Robotic workflow ${index + 1}`,
  long_description: `Automates a distinctive workflow ${index + 1}.`,
  tags: ['Artificial Intelligence'],
  batch: 'Winter 2024',
}));
const FOUNDER_HITS = COMPANY_HITS.flatMap((company, index) => [
  {
    first_name: `Alice${index}`,
    last_name: `Founder${index}`,
    company_slug: company.slug,
    batches: ['W24'],
  },
  {
    first_name: `Bob${index}`,
    last_name: `Builder${index}`,
    company_slug: company.slug,
    batches: ['W24'],
  },
]);
...
function profileHtml(slug: string, founders = FOUNDER_HITS): string {
  return founders
    ...
    .map(
      (founder) =>
        `&quot;is_active&quot;:true,&quot;full_name&quot;:&quot;${founder.first_name} ${founder.last_name}&quot;`,
    )
    .join('');
}
...
describe('YC W24 AI oracle', () => {
  it('parses fresh public Algolia credentials from official directory HTML', () => {
    expect(
      parseAlgoliaCredentials(
        '<script>window.AlgoliaOpts = {"app":"APP123","key":"abcdefghijklmnopqrstuvwxyz"};</script>',
      ),
  ...
  it('ignores companies without public founder rows and validates envelopes', () => {
    const result = combineDirectoryResults(
      { hits: [...COMPANY_HITS, { name: 'No Founder', slug: 'none' }] },
      { hits: FOUNDER_HITS },
    );
  ...
  it('performs two directory reads and two index queries without live HTTP', async () => {
    ...
    const fetchFn = async (
      ...
      if (url.endsWith('/companies'))
        return new Response(
          '<script>window.AlgoliaOpts = {"app":"APP123","key":"companykeycompanykeycompanykey"};</script>',
        );
      if (url.endsWith('/founders'))
        return new Response(
          '<script>window.AlgoliaOpts = {"app":"APP123","key":"founderkeyfounderkeyfounderkey"};</script>',
        );
      if (url.includes('YCCompany_production'))
        return new Response(JSON.stringify({ hits: COMPANY_HITS }));
      if (url.includes('YCUsers_production'))
        return new Response(JSON.stringify({ hits: FOUNDER_HITS }));
    ...
    const result = await fetchYcW24AiCompanies({
      fetchFn,
  ...
  it('keeps an untagged company whose one-liner reads AI and drops a non-AI company', async () => {
    const hits = [
      ...COMPANY_HITS,
      {
        name: 'Maplike',
        slug: 'maplike',
        one_liner: 'AI Agents for Location',
        long_description: 'Keeps map data current.',
        tags: [],
        batch: 'Winter 2024',
      },
      {
        name: 'Pure Fintech',
        slug: 'pure-fintech',
        one_liner: 'Payments for restaurants',
        long_description: 'Card processing for hospitality.',
        tags: ['Fintech'],
        batch: 'Winter 2024',
      },
    ];
    ...
    const fetchFn = async (input: string | URL | Request): Promise<Response> => {
      ...
      if (url.includes('YCUsers_production'))
        return new Response(JSON.stringify({ hits: founders }));
  ...
  it('classifies AI focus by tag or visible text, without substring false positives', () => {
    ...
    expect(isAiFocused({ ...base, longDescription: 'We fine-tune machine learning models.' })).toBe(
      true,
    );
    expect(isAiFocused({ ...base, oneLiner: 'Air freight from Shanghai' })).toBe(false);
    expect(isAiFocused({ ...base, oneLiner: 'Payments for restaurants', tags: ['Fintech'] })).toBe(
      false,
    );
  });
});
