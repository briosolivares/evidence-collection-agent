...
import { grade } from './stub/grader/grader.js';
import { fetchOracle } from './stub/oracle/oracle.js';
...
describe('runEvals', () => {
  it('k trials produce k distinct run dirs, per-trial grades, and correct aggregation', async () => {
    const report = await runEvals([stubTask()], 3, { runTask: makeFakeRunTask(baseDir) });
  ...
  it('a trial without the deliverable fails grading and flips task-pass to false', async () => {
    ...
    const report = await runEvals([stubTask()], 2, { runTask: flaky });
  ...
  it('rejects k < 1, an empty task list, and a grader returning no assertions', async () => {
    const deps = { runTask: makeFakeRunTask(baseDir) };
