...
- Declare `getAccess(input)` on new tools — it drives the scheduler, deriving `{reads, writes, exclusive}` from validated input (parallel reads ≤5; conflicting writes serialize). `readOnly` is a compatibility fallback only. A tool with no `getAccess`, or one whose declaration throws, is treated as exclusive — degrading to serial execution, never to unsafe parallelism.
