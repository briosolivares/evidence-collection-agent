...
import {
  ...
  sendBoundedIpc,
  serializedSize,
  structuredError as structuredIpcError,
  truncateUtf8,
} from './ipcShared.mjs';
...
/** Shared structured-error shape, with every field redaction-wrapped parent-side. */
function structuredError(thrown: unknown): BrowserProgramError {
  return structuredIpcError(thrown, redactBrowserCapabilities);
}
 ...
 *
 * The process-lifecycle machine (byte-capped capture, SIGTERM→grace→SIGKILL,
 * stray-kill, stream-drain deadline, settle-once, watchdog wiring) is the
 * shared `superviseBoundedChildProcess`; this file owns the fork+IPC
 * handshake, the whole message protocol, and result mapping.
 */
export async function runBrowserProgram(
  ...
  try {
    child = fork(CHILD_MODULE, [], {
      ...
      // Always true here: the win32 guard above already returned.
      detached: true,
      serialization: 'json',
    });
  } catch (error) {
    await watchdog.disarm().catch(() => undefined);
    return immediateResult(startedAt, { status: 'failed', error: structuredError(error) });
  }
  ...
  const supervision = superviseBoundedChildProcess<TerminalOutcome>({
    child,
    watchdog,
    abortSignal: options.abortSignal,
    maxOutputBytes: options.maxOutputBytes,
    // A capture that exactly fills the byte budget may still finish cleanly;
    // only a chunk that overflows it (or arrives once the budget is spent)
    // reclassifies the program.
    terminateOnExactFill: false,
    timings: {
      timeoutMs: options.timeoutMs,
      terminateGraceMs: TERMINATE_GRACE_MS,
      strayKillDelayMs: STRAY_KILL_DELAY_MS,
      drainDeadlineMs: DRAIN_DEADLINE_MS,
      terminationDeadlineMs: TERMINATION_DEADLINE_MS,
    },
    outcomes: {
      ...
      exitedWithoutOutcome: (code, signal) => ({
        status: 'failed',
        error: protocolError(
    ...
    replaceOutcome: (current, next) =>
      next.status === 'output_limit_exceeded' &&
      (current.status === 'exited' || current.status === 'failed'),
  });

  const beginTermination = supervision.terminate;
  ...
  const maybeStartProgram = (): void => {
    if (
      ...
      startSent ||
      supervision.settled ||
      supervision.terminationStarted
    ) {
      return;
    }
  ...
  const sendToChild = (message: Record<string, unknown>): boolean =>
    sendBoundedIpc(message, {
      maxBytes: BROWSER_PROGRAM_LIMITS.maxIpcMessageBytes,
      isConnected: () => child.connected,
      send: (payload) =>
        child.send(payload as Record<string, unknown>, (error) => {
          if (error && !supervision.settled) {
        ...
        beginTermination({
          ...
          error:
            kind === 'oversized'
              ...
              : kind === 'closed'
                ? protocolError('browser-program IPC channel closed unexpectedly')
  ...
  const replyToCdp = (id: number, ok: boolean, valueOrError: unknown): void => {
    if (supervision.settled || supervision.hasOutcome) return;
    if (ok) {
      let value: unknown;
      try {
    ...
    const error: BrowserProgramError = {
      ...
      message:
        typeof message.error.message === 'string'
          ? truncateUtf8(redactBrowserCapabilities(message.error.message), MAX_ERROR_MESSAGE_BYTES)
  ...
  child.once('error', (error) => {
    if (child.pid === undefined) {
      supervision.forceOutcome({ status: 'failed', error: structuredError(error) });
  ...
  child.once('disconnect', () => {
    if (!supervision.hasOutcome && !supervision.exitObserved) {
      beginTermination({
        status: 'protocol_error',
  ...
  const finalOutcome: TerminalOutcome = completion.outcome ?? {
    status: 'failed',
