...
- When the request explicitly enumerates a value set (for example specific organizations, categories, or class years), declare the matching column as type enum with exactly that allowed set, and put required coverage of the enumerated values in contentExpectations for the judge to assess as covered or credibly blocked. Allowed values constrain fabrication; required coverage remains a judgment question so truthful partial results stay possible.
...
- Do not add inferred expected-value sets or entity lists, availability claims, domain heuristics, or requirements that merely seem desirable.
