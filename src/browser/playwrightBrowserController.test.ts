 ...
 * through the same target-pinned command session production uses. This never
 * touches this controller's own selected pointer.
 */
async function closeTrackedPageForCleanup(
  controller: BrowserController,
  pageId: string,
): Promise<void> {
  const session = await controller.openCommandSession(pageId);
  try {
