...
import { BrowserbaseBrowserSessionProvider } from './browserbaseBrowserSessionProvider.js';
...
describe('createBrowserSessionProvider', () => {
  it('builds a LocalChromeBrowserSessionProvider for a local env', () => {
    const provider = createBrowserSessionProvider({ env: {}, profileDir: PROFILE_DIR });
...
describe('describeBrowserProvider', () => {
  it('names the profile dir for local', () => {
    expect(describeBrowserProvider({ env: {}, profileDir: PROFILE_DIR })).toContain(PROFILE_DIR);
  });
...
describe('formatBrowserStartupError', () => {
  it('includes Chrome-install guidance for a not-found-shaped local message', () => {
    const formatted = formatBrowserStartupError('local', 'Executable doesn\'t exist at /no/chrome');
  ...
  it('omits install guidance for a local message that is not not-found-shaped', () => {
    const formatted = formatBrowserStartupError('local', 'Chrome crashed unexpectedly');
  ...
  it('always ends with the underlying message, local branch', () => {
    const formatted = formatBrowserStartupError('local', 'some underlying failure detail');
  ...
  it('mentions BROWSERBASE_API_KEY and the local fallback, never Chrome install, browserbase branch', () => {
    const formatted = formatBrowserStartupError('browserbase', 'session limit reached');
