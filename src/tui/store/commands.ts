...
export function availableCommands(evalsEnabled = true): readonly SlashCommand[] {
  return evalsEnabled ? SLASH_COMMANDS : SLASH_COMMANDS.filter((entry) => entry.name !== '/evals');
}

/** The registry entry whose name is exactly `command`, if any. */
export function findCommand(command: string, evalsEnabled = true): SlashCommand | undefined {
 ...
 */
export function filterCommands(input: string, evalsEnabled = true): readonly SlashCommand[] {
  if (!input.startsWith('/') || /\s/.test(input)) return [];
  const prefix = input.toLowerCase();
  return availableCommands(evalsEnabled).filter((entry) => entry.name.startsWith(prefix));
}
