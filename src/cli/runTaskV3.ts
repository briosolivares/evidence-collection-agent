...
import { assertJavaScriptPolicy } from '../browser/browserJavaScript.js';
...
export async function runTaskV3(
  taskText: string,
...
export async function resumeTaskV3(
  runDir: string,
...
function buildFreshConfiguration(
  ...
  if (
    config.harness?.contractAuthor !== undefined &&
    config.harness.contractAuthor !== 'initializer'
  ) {
    throw new Error(
      'runTaskV3 accepts only initializer-authored immutable contracts',
    );
  }
...
function assertResumeConfigurationMatches(
  ...
  if (
    ...
    throw new Error(
      `resume configuration mismatch for harness.contractAuthor: v3 checkpoint ` +
        `requires "initializer", caller supplied ${JSON.stringify(
          config.harness.contractAuthor,
        )}`,
    );
  }
}
...
function usableStartUrl(startUrl: string | undefined): string | undefined {
...
export function isV3CheckpointVersion(version: number): boolean {
  return version === V3_CHECKPOINT_VERSION;
}
