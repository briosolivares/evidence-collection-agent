...
// cannot lie.
//
// Why --manual exists: Google refuses its sign-in flow outright in a
// browser it identifies as automated ("Couldn't sign you in — This browser
// or app may not be secure"), and Playwright's launch is unmistakably
// automated: it passes --enable-automation and opens a remote-debugging
// port. No flag combination reliably talks Google out of that, so the
// default local mode works for X but cannot work for Google. The session,
// though, is just cookies in the profile directory: sign in with an
// ordinary Chrome pointed at that same --user-data-dir, quit it, and every
// later automated launch inherits a session Google already trusts. It is
// the sign-in *flow* that is blocked, not the cookie. --manual has no
// remote analogue: a Browserbase browser is only reachable through Live
// View, which is what the Browserbase flow uses.
...
import { resolveRealChromePath } from '../browser/localChromeExecutable.js';
import {
...
import { HEADED_LANE_SERVICES, allLoggedIn } from './loginProbe.js';
...
/** The one sentence every mode ends on, plus the matching exit code — so
 * LOGIN OK / NOT READY reads identically whichever path produced it. */
function reportVerdict(ready: boolean, okDetail: string, notReadyDetail: string): void {
  console.log(ready ? `\nLOGIN OK — ${okDetail}` : `\nNOT READY — ${notReadyDetail}`);
...
async function main(): Promise<void> {
  ...
  console.log(
    '(the exact place authenticated eval trials use — logins anywhere else do not count)',
  );
  ...
  async function verifyAll(context: BrowserContext): Promise<boolean> {
    const statuses = await probeServices(context, HEADED_LANE_SERVICES, probeStatusPrinter());
  ...
  async function runManual(): Promise<void> {
    ...
    console.log('\nVerifying…');
    reportVerdict(
      await verifyAfterRelease(),
      `${paths.profileDir} is ready for headed batches.`,
      'the session did not stick. Re-run `npm run login -- --manual`.',
    );
  }
  ...
  async function runBrowserbase(): Promise<void> {
    ...
    if (checkOnly) {
      ...
      reportVerdict(
        allLoggedIn(statuses),
        'the Browserbase context is ready for authenticated batches.',
        'run `npm run login` to sign in through Live View.',
      );
      return;
    }
    ...
    reportVerdict(
      ready,
      'the logins survived closing and reopening the context, which is what ' +
        'an authenticated trial will do.',
      'the logins did not survive the session boundary. Re-run ' +
  ...
  if (provider === 'browserbase') {
    ...
    try {
      if (checkOnly) {
        ...
        reportVerdict(
          await verifyAll(context),
          `${paths.profileDir} is ready for headed batches.`,
          'run `npm run login` to log in, or `npm run login -- --manual` ' +
        ...
        await openServiceTabs(context, HEADED_LANE_SERVICES);
...
// Runs only as an entry point (`npm run login`). Importing this module — the
// tests do, for the manual-launch helpers — must not start a login flow.
