...
describe('collapseStaleObservations', () => {
  ...
  it('never stubs a failed observation and never counts it toward the kept window', () => {
    ...
    expect(collapseStaleObservations(twoPlusError)).toBe(twoPlusError);
    ...
    const view = collapseStaleObservations(threePlusError);
    expect(resultBlock(view, 2).content).toContain(COLLAPSED_MARKER);
  ...
  it('recovers the URL/title header from an offloaded result via its preview', () => {
    ...
    const stub = resultBlock(collapseStaleObservations(messages), 2);
    expect(stub.content).toContain('URL: https://site.test/page-1');
    expect(stub.content).toContain('Title: Page 1');
  });
  ...
  it('keeps already-stubbed messages byte-identical as new observations displace the window', () => {
    ...
    const viewOfThree = collapseStaleObservations(conversation(3));
    const viewOfFour = collapseStaleObservations(conversation(4));

    expect(JSON.stringify(viewOfFour[2])).toBe(JSON.stringify(viewOfThree[2]));
    expect(resultBlock(viewOfFour, 4).content).toContain(COLLAPSED_MARKER);
  ...
  it('stubs only the observe result inside a mixed parallel batch', () => {
    ...
    expect((batch.content[0] as ToolResultBlock).content).toContain(COLLAPSED_MARKER);
